...
let validationSchema=Yup.object().shape({
  ...
  function  validate(){
    console.log(formik)
    if((!formik.errors.name && formik.values.name)&&(!formik.errors.phone&&formik.values.phone)
       &&(!formik.errors.email&&formik.values.email)&& (!formik.errors.password && formik.values.password)&& (!formik.errors.rePassword &&formik.values.rePassword))
      {
        setNoErro(false)
        }else{
         setNoErro(true)
  ...
  return (
    ...
    <h2 className=' flex items-center my-2 gap-2 text-green-600 font-bold sm:mt-4 text-[24px] sm:text-[30px]'>
...
<form className="w-[90%]  lg:w-[50%] sm:pt-4 " onSubmit={formik.handleSubmit}>
 ...
 {apiError && <p className='p-2 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400'>{apiError}</p> }
    ...
    <div className="relative mb-2 z-0 w-full  sm:mb-2 group">
